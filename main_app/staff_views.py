    ...
    if request.method == 'POST':
        if form.is_valid():
            try:
                ...
                return redirect(reverse('staff_add_note'))
            except Exception as e:
                messages.error(request, f"Could not add note: {str(e)}")
        else:
            messages.error(request, "Form has errors!")
    
    return render(request, "staff_template/staff_add_note.html", context)
...
def staff_delete_assignment(request, assignment_id):
    try:
        ...
        messages.error(request, f"Could not delete assignment: {str(e)}")
    return redirect(reverse('staff_add_assignment'))
...
def staff_delete_note(request, note_id):
    try:
        ...
        messages.error(request, f"Could not delete note: {str(e)}")
    return redirect(reverse('staff_add_note'))
